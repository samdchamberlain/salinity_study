---
title: 'Supplemental Materials: Dynamic ecosystem function during wetland salinization:
  differential effects on methane biogeochemistry and ecosystem productivity.'
...
```{r fig2, fig.cap = "Monthly cumulative fluxes of methane (*F~CH4~*), gross ecosystem photosynthesis (*GEP*), ecosystem respiration (*ER*), as well as mean salinity at the salinizaiton wetland. The wetland was one year old and fully vegetated by 2012.", fig.width=3, fig.height=7, dpi=600}
...
```{r fig4, fig.cap="Time series of daily methane fluxes (*F~CH4~*), gross ecosystem photosynthesis (*GEP*), net ecosystem exchange (*NEE*), and salinity at the reference wetland and the wetland that experienced salinization.", fig.height = 5, fig.width = 7, dpi=600}
time_series <- theme_bw() +
  theme(panel.grid.minor=element_blank(), panel.grid.major.y = element_blank(),
...
mc_wm_ta <- tr_timeseries(TA_anom, CH4na_anom, data_list = by_year, type = "shuffled", 
                           lags = 72, bins = 10, runs = 100, alpha = 0.05, normalize = T)
# for CH4 -> TA
...
```{r fig6, fig.cap="Relative transfer entropy (*TE*) from (top panel) *GEP* to *F~CH4~* and (bottom panel) *F~CH4~* to *GEP* over multiple growing seasons (DOY 100-300). The red line represents significance threshold (*P* < 0.05) based on Monte Carlo reshuffling of the time series.", fig.height=4, fig.width=7.5, dpi=600}
