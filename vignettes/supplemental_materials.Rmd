...
source("R/peat19_processing.R") #mature wetland baseline (no salinity rise)
```

```{r fig1, fig.cap="Spurious relative mutual information (MI) is impacted by the total number of available obervations in each monthly calculation. All values presented are reshuffled in the same manner as Monte Carlo reshufflings for significance testing in the main manuscript. Blue line represents a loess fit for the presented data.", fig.height= 5, fig.width=6, dpi=600}
...
```{r fig3, fig.cap="Time series of daily methane fluxes (*F~CH4~*), gross ecosystem photosynthesis (*GEP*), net ecosystem echange (*NEE*), and salinity at the reference wetland and salinization wetland.", fig.height = 5, fig.width = 7, dpi=600}
...
```{r fig4, fig.cap="Relative transfer entropy (***TE***) from (top panel) half-hourly *Ta* to *F~CH4~* and (bottom panel) *F~CH4~* to *Ta* over multiple growing seasons (DOY 100-300). The red line represents significance threshold (*P* < 0.05) based on Monte Carlo reshuffling of the time series. Based on ~a priori~ knowledge only changes in *Ta* should induce changes in *F~CH4~*, but not vice versa.", fig.height=4, fig.width=7.5, dpi=600}
...
```{r fig5, fig.cap="Relative transfer entropy (***T***) from (top panel) half-hourly *GEP* to *F~CH4~* and (bottom panel) *F~CH4~* to *GEP* over multiple growing seasons (DOY 100-300). The red line represents significance threshold (*P* < 0.05) based on Monte Carlo reshuffling of the time series.", fig.height=4, fig.width=7.5, dpi=600}
