...
wm_rs <- (peat6_all$wm - mean(peat6_all$wm, na.rm = T))/sd(peat6_all$wm, na.rm = T)
wq_rs <- (peat6_all$wq - mean(peat6_all$wq, na.rm = T))/sd(peat6_all$wq, na.rm = T)


mutual_info(wm_rs, wq_rs)
...
clipped_monthly$ET_CH4_mi <- mi_timeseries(wq_gf, wm, data_list = by_month)
clipped_monthly$Tair_CH4_mi <- mi_timeseries(TA.x, wm, data_list = by_month)

#create significance thresholds across entire measurement period
rand_ch4 <- sample(peat6_all$wm, length(peat6_all$wm), replace = F)
rand_Tair <- sample(peat6_all$TA.x, length(peat6_all$TA.x), replace = F)
rand_GPP <- sample(peat6_all$gpp_ANNnight, length(peat6_all$gpp_ANNnight), replace = F)

mutual_info(rand_Tair, rand_ch4)
mutual_info(peat6_all$TA.x, peat6_all$wm)

mutual_info(rand_GPP, rand_ch4)
mutual_info(peat6_all$gpp_ANNnight, peat6_all$wm)

sig_gpp <- mi_confidence(check$gpp_ANNnight, check$wm, runs = 1000)
sig_Tair <- mi_confidence(peat6_all$TA.x, peat6_all$wm, runs = 1000)

peat6_all$month <- month(peat6_all$datetime)
check <- subset(peat6_all, year == 2012 & month == 7)
...
# #plot that needs to be dealt with later
# clipped_monthly$GPP_CH4_c <- corr_timeseries(gpp_ANNnight*-1, wm, data_list = by_month, method = "spearman")
# clipped_monthly$ET_CH4_c <- corr_timeseries(wq_gf, wm, data_list = by_month, method = "spearman")
# clipped_monthly$Tair_CH4_c <- corr_timeseries(TA.x, wm, data_list = by_month, method = "spearman")

# # only look at MI with at least 500 observations
# clipped_monthly$GPP_CH4_mi <- ifelse(clipped_monthly$CH4_obs > 400, clipped_monthly$GPP_CH4_mi, NA)
# clipped_monthly$ET_CH4_mi <- ifelse(clipped_monthly$CH4_obs > 400, clipped_monthly$ET_CH4_mi, NA)
# clipped_monthly$Tair_CH4_mi <- ifelse(clipped_monthly$CH4_obs > 400, clipped_monthly$Tair_CH4_mi, NA)

a <- ggplot(clipped_monthly) +
  geom_hline(yintercept = mean(sig_gpp), size = 0.5, color = "dark green", linetype = "dashed") +
  geom_hline(yintercept = mean(sig_Tair), size = 0.5, color = "red", linetype = "dashed") +
  geom_point(aes(x = datetime, y = Tair_CH4_mi), color = "red", alpha = 0.5) +
  geom_line(aes(x = datetime, y = ma(Tair_CH4_mi, 2)), color = "red", size = 1) +
  geom_point(aes(x = datetime, y = GPP_CH4_mi), color = "dark green", alpha = 0.5) +
  geom_line(aes(x = datetime, y = ma(GPP_CH4_mi, 2)), color = "dark green", size = 1) +
  ylab("Mutual information") + theme_bw() + xlab("")

# b <- ggplot(subset(peat6_daily, year > 2011 & year < 2018), aes(x=datetime, y=mgCH4)) +
#   geom_point(size = 0.5) + theme_bw() + xlab("") +
#   ylab("CH4 flux (g C)")
# 
# c <- ggplot(subset(peat6_daily, year > 2011 & year < 2018), aes(x=datetime, y=gCO2)) +
#   geom_point(color = "dark green", size = 0.5) +
#   theme_bw() + xlab("") +
#   ylab("GEP flux (g C)")

d <- ggplot(subset(peat6_daily, year > 2011 & year < 2018), aes(x=datetime, y=Cond)) +
  geom_line(color = "blue") +
  ylab("Conductivity (mS)") + theme_bw()
  
grid.arrange(a, d, ncol=1)
