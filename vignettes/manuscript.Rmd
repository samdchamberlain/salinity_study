...
sig_gpp <- conf_series(gpp_ANNnight, wm, data_list = by_month, runs = 100, alpha = 0.01)
# mean sig_gpp across all months at p < 0.01: 0.034

sig_Tair <- conf_series(TA.x, wm, data_list = by_month, runs = 100, alpha = 0.01)
...
mc_wm_gpp <- tr_timeseries(GPP_anom, CH4na_anom, data_list = by_year, type = "shuffled", 
                           lags = 120, bins = 10, runs = 5, alpha = 0.05, normalize = T)
# for TA -> CH4
...
rise_years <- peat6_daily %>%
  ...
  select(datetime, mgCH4, gGEP, gER, mET, PAR, H, Tair, Tw, Ts, mVPD, PA, u., Rain, WTD)
