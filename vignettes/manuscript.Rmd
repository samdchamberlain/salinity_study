...
## Couplings and Controls of Ecosystem
We used *MI* to identify coupling strengths and *TE* to identify causal interactions between variables and the time scales at which they occur. Mutual information is used to identify the amount of information shared between two variables *X* and *Y*, and can be described using the following equation:
...
While *MI* identifies coupling strength [@sturtevant_identifying_2016; @chamberlain_soil_2018] between variables it does not ascribe directionality, or causality, to the relationship. Transfer entropy is used to ascribe causality between variables and quantifies the extent to which knowledge of a variables *X* reduced the uncertainty of the present state of *Y* given knowledge of the history of *Y* itself. If a value of *TE* is above some signficance threshold (described below) it is deduced that *X* provides information to the current state of *Y* [@@ruddell_ecohydrologic_2009]. Transfer entropy can also be described by joint and marginal entropies incorporating time lags as follows [@knuth_revealing_2013; @ruddell_ecohydrologic_2009]:
$$T(X>Y, \tau) = H(X_{t-\tau}, Y_{t-1}) + H(Y_{t}, Y_{t-1}) - H(Y_{t-1}) + H(X_{t-\tau}, Y_{t}, Y_{t-1})$$
Following @ruddell_ecohydrologic_2009
